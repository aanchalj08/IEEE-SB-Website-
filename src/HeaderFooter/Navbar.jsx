...
const Navbar = () => {
  ...
  return (
    <div>
       <div className={!shownav ? 'w-screen h-screen bg-black opacity-85 z-50 fixed left-[1800px] transition-all':"w-screen h-screen bg-black opacity-85 z-50 fixed left-0 transition-all"}>
        <div className="mt-6 ml-6 " onClick={()=>setShownav(!shownav)}>
       <FontAwesomeIcon icon={faXmark} size="2xl" style={{color: "#fafcff",}} />
       </div>
      ...
      <nav role="navigation" className={` z-10 flex justify-between items-center mx-12 font-serif ${isNavbarFixed ? "fixed top-0 left-[-47px] right-[-47px]  bg-white z-50 shadow-lg transition-all " : "relative"}`}>
